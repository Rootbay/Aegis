    ...
    <div class="space-y-2 pr-2">
      {#each roles as role (role.id)}
        <div
          ...
          onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); startEditRole(role); } }}
        >
    ...
    {#if editingRole}
      ...
      <div class="space-y-4">
        <div class="flex items-center gap-4">
          <label for="roleName" class="text-sm font-medium text-muted-foreground w-24">Role Name</label>
          <input
            type="text"
            id="roleName"
        ...
        <div class="flex items-center gap-4">
          <label for="roleColor" class="text-sm font-medium text-muted-foreground w-24">Role Color</label>
          <input
            type="color"
            id="roleColor"
          ...
          />
          <span class="text-sm text-muted-foreground">{editingRoleColor.toUpperCase()}</span>
        </div>

        <label class="flex items-center space-x-3">
          <input type="checkbox" bind:checked={editingRoleHoist} class="form-checkbox h-5 w-5 text-highlight-100 bg-muted border-border rounded" />
          <span class="text-muted-foreground">Display role members separately from online members</span>
        </label>

        <label class="flex items-center space-x-3">
          <input type="checkbox" bind:checked={editingRoleMentionable} class="form-checkbox h-5 w-5 text-highlight-100 bg-muted border-border rounded" />
          <span class="text-muted-foreground">Allow anyone to @mention this role</span>
        </label>

        {#each Object.entries(permissionCategories) as [categoryName, permissionsInCat] (categoryName)}
          <div class="pt-4">
            <h4 class="text-lg font-semibold text-foreground mb-2">{categoryName}</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2">
              {#each permissionsInCat as permissionKey (permissionKey)}
                <label class="inline-flex items-center p-2 rounded-md hover:bg-muted transition-colors">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span class="ml-3 text-muted-foreground">{allPermissions[permissionKey]}</span>
                </label>
              {/each}
            </div>
          </div>
