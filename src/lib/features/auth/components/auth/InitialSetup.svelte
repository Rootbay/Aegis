<script lang="ts">
  ...
  const storedSecurityQuestions = $derived(
    () => $authPersistenceStore.securityQuestions ?? []
  );
  ...
  const recoveryFormReady = $derived(() => {
    if (recoveryQuestionPrompts.length === 0) return false;
    if (answeredRecoveryQuestions < 2) return false;
    if (!recoveryPassword || !recoveryPasswordConfirm) return false;
    if (recoveryPassword !== recoveryPasswordConfirm) return false;
    if (recoveryPasswordValidation) return false;
    if (requireTotpOnUnlock && recoveryTotp.length !== 6) return false;
    return true;
  });
  ...
  async function handleSecurityQuestionRecovery(event: Event) {
    ...
    if (questions.length === 0) {
      recoveryError = 'Security questions are not configured for this identity.';
      return;
    }

    const trimmedAnswers = questions.reduce<Record<string, string>>((acc, item) => {
  ...
  const inOnboardingFlow = $derived(status === 'needs_setup' || status === 'setup_in_progress');
  const hasStoredSecurityQuestions = $derived(storedSecurityQuestions.length > 0);

  $effect(() => {
    if (!showRecoveryForm) return;
    const prompts = new Set(recoveryQuestionPrompts);
    const filteredEntries = Object.entries(securityQuestionAnswers).filter(([question]) =>
      prompts.has(question)
    );
    if (filteredEntries.length !== Object.keys(securityQuestionAnswers).length) {
      securityQuestionAnswers = Object.fromEntries(filteredEntries);
    }
  });
  ...
  $effect(() => {
    if (!inOnboardingFlow && (status === 'recovery_ack_required' || pendingRecovery)) {
      activeView = 'recovery';
    }
  });
</script>
    ...
    <div class="mx-auto w-full max-w-3xl space-y-8">
      <div class="flex flex-wrap justify-center gap-3">
        ...
        <Button
          ...
          class="flex items-center gap-2"
          onclick={() => { resetRecoveryFormFields(); switchView('recovery'); showRecoveryForm = false; }}
                            ...
                            <form class="space-y-4" onsubmit={handleSecurityQuestionRecovery}>
                                  ...
                                  {#each recoveryQuestionPrompts as prompt, index (prompt)}
                                    <div class="space-y-1.5">
                                      ...
                                      <Input
                                        ...
                                        oninput={(event) => {
                                          const target = event.currentTarget as HTMLInputElement
                                          securityQuestionAnswers = { ...securityQuestionAnswers, [prompt]: target.value }
                                        }}
                                        autocomplete="off"
                                      />
                                    </div>
                              ...
                              <div class="space-y-2">
                                ...
                                {#if recoveryPasswordValidation && recoveryPassword}
                                  <p class="text-xs text-destructive">{recoveryPasswordValidation}</p>
                                {/if}
                              </div>
                              ...
                              <div class="space-y-2">
                                ...
                                {#if recoveryPassword && recoveryPasswordConfirm && recoveryPassword !== recoveryPasswordConfirm}
                                  <p class="text-xs text-destructive">Passwords must match.</p>
                                {/if}
                              </div>
