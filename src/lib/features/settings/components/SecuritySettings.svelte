<script lang="ts">
    import { Lock, Shield, Smartphone, Clock, Trash2, AlertTriangle } from '@lucide/svelte';
    ...
    function updateSessionTimeout() {
      ...
      toasts.addToast(`Session timeout updated to ${sessionTimeoutMinutes} minutes.`, 'success');
    }
  
    function toggleTotpRequirement() {
      authStore.setRequireTotpOnUnlock(requireTotpOnUnlock);
      toasts.addToast(`2FA requirement ${requireTotpOnUnlock ? 'enabled' : 'disabled'} for unlock.`, 'success');
    }
  ...
  <div class="space-y-6">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-sm text-zinc-400">Manage your account security and trusted devices.</p>
      </div>
    </div>
    ...
    <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-4">
      ...
      <p class="text-sm text-zinc-400">Automatically lock your account after a period of inactivity.</p>
    ...
    <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-4">
      ...
      <p class="text-sm text-zinc-400">Require authenticator code when unlocking your account.</p>
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm">Require 2FA on unlock</p>
          <p class="text-xs text-zinc-500">Authenticator codes will be required even when using your password</p>
        </div>
        <label class="relative inline-flex items-center cursor-pointer">
          ...
          />
          <div class="w-11 h-6 bg-zinc-600 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary/25 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
        </label>
      </div>
    </div>
    ...
    <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-4">
      ...
      <p class="text-sm text-zinc-400">Devices that can access your account without additional verification.</p>
          ...
          {#each trustedDevices as device (device.id)}
            <div class="flex items-center justify-between p-3 bg-zinc-800/50 rounded-lg">
              <div>
                ...
                <p class="text-xs text-zinc-500">Last used: {formatDate(device.lastUsed)}</p>
                {#if device.userAgent}
                  <p class="text-xs text-zinc-600 truncate max-w-xs">{device.userAgent}</p>
                {/if}
              </div>
              <button 
    ...
    <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-4">
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center justify-between text-sm">
          ...
          <span class="font-medium" class:text-green-400={requireTotpOnUnlock} class:text-yellow-400={!requireTotpOnUnlock}>
