<script lang="ts">
  ...
  import { mdiPlus, mdiPencil, mdiTrashCan, mdiCheck, mdiClose, mdiPound, mdiVolumeHigh } from '@mdi/js';
    ...
    {#each channels as channel (channel.id)}
      <div class="bg-muted/50 p-3 rounded-lg flex items-center justify-between">
        <span class="font-medium flex items-center gap-2">
          <Icon data={channel.channel_type === 'text' ? mdiPound : mdiVolumeHigh} size="5" />
          {channel.name}
        </span>
        <div>
          <button onclick={() => startEditChannel(channel)} class="text-muted-foreground hover:text-foreground mr-2"><Icon data={mdiPencil} size="5" /></button>
          <button onclick={() => deleteChannel(channel.id)} class="text-destructive hover:text-destructive/80"><Icon data={mdiTrashCan} size="5" /></button>
        </div>
      </div>
