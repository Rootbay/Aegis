...
<script lang="ts">
  ...
  function buildMenuEntries(currentServer: Server, isMuted: boolean): MenuEntry[] {
    if (!currentServer) return [];

    const isOwner = currentServer.owner_id === $userStore.me?.id;

    const entries: MenuEntry[] = [
      { type: 'item', id: 'mark-read', label: 'Mark As Read', action: 'mark_as_read' },
      {
        type: 'item',
        id: 'toggle-mute',
      ...
      { type: 'item', id: 'notifications', label: 'Notification Settings', action: 'notification_settings' },
      { type: 'separator', id: 'divider-utilities' },
      { type: 'item', id: 'copy-id', label: 'Copy Server ID', action: 'copy_server_id' },
      { type: 'item', id: 'view-icon', label: 'View Icon', action: 'view_icon' },
      { type: 'item', id: 'view-raw', label: 'View Raw', action: 'view_raw' }
    ];

    if (isOwner) {
      entries.push({ type: 'separator', id: 'divider-owner' });
      entries.push({ type: 'item', id: 'server-settings', label: 'Server Settings', action: 'server-settings' });
      entries.push({ type: 'item', id: 'create-channel', label: 'Create Channel', action: 'create_channel' });
      entries.push({ type: 'item', id: 'create-category', label: 'Create Category', action: 'create_category' });
      entries.push({ type: 'item', id: 'create-event', label: 'Create Event', action: 'create_event' });
    } else {
      entries.push({ type: 'separator', id: 'divider-member' });
      entries.push({ type: 'item', id: 'invite-people', label: 'Invite People', action: 'invite_people' });
