...
<div class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[60]">
  <div class="bg-card p-6 rounded-lg shadow-lg w-full max-w-md relative">
    ...
    {#if errorMessage}
        <div class="bg-red-500/20 border border-red-500 text-red-300 p-4 rounded-lg mb-4">
            <p>{errorMessage}</p>
        </div>
    {/if}
    <div id="qr-reader" class="w-full rounded-lg overflow-hidden border-2 border-zinc-700"></div>
