...
describe("formatTimestamp", () => {
  ...
  it("returns fallback when timestamp is missing", () => {
    expect(formatTimestamp(undefined, { now: reference, locale })).toBe(
      "Just now",
    );
  });
  ...
  it("returns null when fallback is explicitly null", () => {
    expect(
      formatTimestamp(undefined, { now: reference, fallback: null, locale }),
    ).toBe(null);
  });

  it("formats values within an hour as relative minutes", () => {
    const oneMinuteAgo = new Date(reference.getTime() - 60_000);
    expect(formatTimestamp(oneMinuteAgo, { now: reference, locale })).toBe(
      "1 minute ago",
    );
  });
  ...
  it("formats values within a day as relative hours", () => {
    ...
    expect(formatTimestamp(twoHoursAhead, { now: reference, locale })).toBe(
      "in 2 hours",
    );
  });
  ...
  it("formats values older than five weeks as a calendar date", () => {
    ...
    expect(formatTimestamp(pastDate, { now: reference, locale })).toBe(
      "Oct 15, 2023, 9:30 AM",
    );
  });
